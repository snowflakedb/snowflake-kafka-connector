...
public class SnowflakeSinkConnectorConfig {
    ...
    public static final String SNOWFLAKE_STREAMING_METADATA_CONNECTOR_PUSH_TIME = "snowflake.streaming.metadata.connectorPushTime";
    ...
    public static final String SNOWPIPE_STREAMING_CLOSE_CHANNELS_IN_PARALLEL = "snowflake.streaming.closeChannelsInParallel.enabled";
    ...
    public static final String SNOWPIPE_STREAMING_CLIENT_PROVIDER_OVERRIDE_MAP = "snowflake.streaming.client.provider.override.map";
    ...
    public static final String SNOWPIPE_STREAMING_USE_USER_DEFINED_DATABASE_OBJECTS = "snowflake.streaming.use.user.defined.database.objects";
    public static final boolean SNOWPIPE_STREAMING_USE_USER_DEFINED_DATABASE_OBJECTS_DEFAULT_VALUE = false;
    ...
    public static final String ERRORS_TOLERANCE_DOC =
        "Behavior for tolerating errors during Sink connector's operation. 'NONE' is set as default" + " and denotes that it will be fail fast. i.e any error will result in an immediate task" +
    ...
    public static final String ERRORS_LOG_ENABLE_DOC =
        "If true, write/log each error along with details of the failed operation and record" + " properties to the Connect log. Default is 'false', so that only errors that are not" +
    ...
    public static final String ERRORS_DEAD_LETTER_QUEUE_TOPIC_NAME_DISPLAY = "Send error records to the Dead Letter Queue (DLQ)";
    ...
    public static final String ENABLE_CHANNEL_OFFSET_TOKEN_VERIFICATION_FUNCTION_DOC =
        "Whether to enable streaming channel offset verification function. The function checks only" + " for incremental offsets (might contain gaps) and might signal false positives in case" +
            " of SMT. Can only be set if Streaming Snowpipe is enabled";
    public static final String ENABLE_TASK_FAIL_ON_AUTHORIZATION_ERRORS = "enable.task.fail.on.authorization.errors";
    ...
    public static final String VALUE_SCHEMA_REGISTRY_CONFIG_FIELD = "value.converter.schema.registry.url";
    public static final Set<String> CUSTOM_SNOWFLAKE_CONVERTERS =
        Set.of("com.snowflake.kafka.connector.records.SnowflakeJsonConverter", "com.snowflake.kafka.connector.records.SnowflakeAvroConverterWithoutSchemaRegistry",
            "com.snowflake.kafka.connector.records.SnowflakeAvroConverter");
    /**
     * Boolean Validator of passed booleans in configurations (TRUE or FALSE). This validator is case
     * insensitive
     */
    public static final ConfigDef.Validator BOOLEAN_VALIDATOR = new ConfigDef.Validator() {
        private final ConfigDef.ValidString validator = ConfigDef.ValidString.in(Boolean.TRUE.toString().toLowerCase(Locale.ROOT), Boolean.FALSE.toString().toLowerCase(Locale.ROOT));
    ...
    public enum KafkaProvider {
        ...
        public static final List<String> PROVIDER_NAMES = Arrays.stream(KafkaProvider.values()).map(kafkaProvider -> kafkaProvider.name().toLowerCase()).collect(Collectors.toList());
        ...
        public static KafkaProvider of(final String kafkaProviderStr) {
            ...
            throw new IllegalArgumentException(String.format("Unsupported provider name: %s. Supported are: %s", kafkaProviderStr, String.join(",", PROVIDER_NAMES)));
        }
    }
    ...
    public enum ErrorTolerance {
         ...
         */
        public static final ConfigDef.Validator VALIDATOR = new ConfigDef.Validator() {
            private final ConfigDef.ValidString validator = ConfigDef.ValidString.in(ErrorTolerance.names());
