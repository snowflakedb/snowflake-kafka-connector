...
public class SnowpipeStreamingV2PartitionChannel implements TopicPartitionChannel {
  ...
  public SnowpipeStreamingV2PartitionChannel(
    ...
    if (Utils.isIcebergEnabled(connectorConfig)) {
      //      String recordContentType = "OBJECT(id_int8 NUMBER(10,0))";
      //      String recordContentType = "object(id_int8 NUMBER(10,0),id_int16 NUMBER(10,0),id_int32
      // NUMBER(10,0),id_int64 NUMBER(19,0),description STRING,rating_float32 FLOAT,rating_float64
      // FLOAT,approval BOOLEAN,array1 ARRAY(NUMBER(19,0),array2 ARRAY(STRING),array3
      // ARRAY(BOOLEAN),array4 ARRAY(NUMBER(19,0),array5 ARRAY(ARRAY(NUMBER(19,0)),nestedRecord
      // object(id_int8 NUMBER(10,0),id_int16 NUMBER(10,0),id_int32 NUMBER(10,0),id_int64
      // NUMBER(19,0),description STRING,rating_float32 FLOAT,rating_float64 FLOAT,approval
      // BOOLEAN),nestedRecord2 object(id_int8 NUMBER(10,0),id_int16 NUMBER(10,0),id_int32
      // NUMBER(10,0),id_int64 NUMBER(19,0),description STRING,rating_float32 FLOAT,rating_float64
      // FLOAT,approval BOOLEAN))";
      List<DescribeTableRow> describeTableRows =
          conn.describeTable(tableName)
              .orElseThrow(() -> new RuntimeException("Table does not exist"));
      String recordContentType = getColumnType(describeTableRows, "RECORD_CONTENT");
      String recordMetadataType = getColumnType(describeTableRows, "RECORD_METADATA");
      conn.createPipeForNonSchematizedIceberg(
          tableName, pipeName, recordMetadataType, recordContentType);
    } else {
      conn.createPipeWithParseJson(tableName, pipeName);
    }
  ...
  private String getColumnType(List<DescribeTableRow> describeTableRows, String columnName) {
    return describeTableRows.stream()
        .filter(row -> row.getColumn().equals(columnName))
        .findFirst()
        .map(DescribeTableRow::getType)
        .orElseThrow(() -> new RuntimeException("Column does not exist"));
  }
