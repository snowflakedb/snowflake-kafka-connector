...
public class StreamingClientProvider {
     ...
     */
    @VisibleForTesting
    public static StreamingClientProvider injectStreamingClientProviderForTests(int createdClientId, Map<String, String> connectorConfig, SnowflakeStreamingIngestClient client) {
    ...
    private StreamingClientProvider(int createdClientId, Map<String, String> connectorConfig, SnowflakeStreamingIngestClient client) {
    ...
    public void createClient(Map<String, String> connectorConfig) {
        ...
        if (isClientValid(this.streamingIngestClient)) {
            LOGGER.error("Replacing previous valid streaming client. ClientName:{}", this.streamingIngestClient.getName());
            this.closeClient();
        }
    ...
    public void closeClient() {
        ...
        try {
            ...
            String message = e.getMessage() == null || e.getMessage().isEmpty() ?
                    ...
                    e.getMessage();
            String cause = e.getCause() == null || e.getCause().getStackTrace() == null ?
     ...
     * Note that this requires createClient() to be run first in order to get the connector config
     *
    ...
    private SnowflakeStreamingIngestClient initStreamingClient(Map<String, String> connectorConfig) {
        String clientName = STREAMING_CLIENT_PREFIX_NAME + connectorConfig.getOrDefault(Utils.NAME, "DEFAULT") + "_" + this.createdClientId;
        ...
        streamingClientProps.putAll(StreamingUtils.convertConfigForStreamingClient(new HashMap<>(connectorConfig)));

        try {
            // Override only if bdec version is explicitly set in config, default to the version set inside Ingest SDK
