...
public class StreamingClientProperties {
    ...
    public static final List<String> LOGGABLE_STREAMING_CONFIG_PROPERTIES =
        Stream.of(Constants.ACCOUNT_URL, Constants.ROLE, Constants.USER, StreamingUtils.STREAMING_CONSTANT_AUTHORIZATION_TYPE).collect(Collectors.toList());
    ...
    public StreamingClientProperties(Map<String, String> connectorConfig) {
        ...
        if (connectorConfig == null) {
            LOGGER.warn("Creating empty streaming client properties because given connector config was empty");
        ...
        this.clientName = STREAMING_CLIENT_PREFIX_NAME + connectorConfig.getOrDefault(Utils.NAME, DEFAULT_CLIENT_NAME);
        ...
        Optional<String> snowpipeStreamingMaxClientLag = Optional.ofNullable(connectorConfig.get(SNOWPIPE_STREAMING_MAX_CLIENT_LAG));
        snowpipeStreamingMaxClientLag.ifPresent(overriddenValue -> parameterOverrides.put(MAX_CLIENT_LAG, String.format("%s second", overriddenValue)));

        // MAX_CHANNEL_SIZE_IN_BYTES can still be set via SNOWPIPE_STREAMING_CLIENT_PROVIDER_OVERRIDE_MAP

        Optional<String> snowpipeStreamingMaxMemoryLimit = Optional.ofNullable(connectorConfig.get(SNOWPIPE_STREAMING_MAX_MEMORY_LIMIT_IN_BYTES));
        snowpipeStreamingMaxMemoryLimit.ifPresent(overriddenValue -> parameterOverrides.put(MAX_MEMORY_LIMIT_IN_BYTES, overriddenValue));
    ...
    private void combineStreamingClientOverriddenProperties(Map<String, String> connectorConfig) {
        ...
        // earlier versions.
        Optional<String> clientOverrideProperties = Optional.ofNullable(connectorConfig.get(SNOWPIPE_STREAMING_CLIENT_PROVIDER_OVERRIDE_MAP));
        Map<String, String> clientOverridePropertiesMap = new HashMap<>();
        clientOverrideProperties.ifPresent(overriddenKeyValuePairs -> {
            LOGGER.info("Overridden map provided for streaming client properties: {}", overriddenKeyValuePairs);
            Map<String, String> overriddenPairs = Utils.parseCommaSeparatedKeyValuePairs(overriddenKeyValuePairs);
            overriddenPairs.forEach((overriddenKey, overriddenValue) -> {
                if (overriddenKey.equalsIgnoreCase(MAX_CLIENT_LAG)) {
                    clientOverridePropertiesMap.put(MAX_CLIENT_LAG, String.format("%s second", overriddenValue));
                } else {
                    clientOverridePropertiesMap.put(overriddenKey.toLowerCase(), overriddenValue);
                }
            });
            overrideStreamingClientPropertyIfSet(clientOverridePropertiesMap, MAX_CLIENT_LAG, SNOWPIPE_STREAMING_MAX_CLIENT_LAG);
            overrideStreamingClientPropertyIfSet(clientOverridePropertiesMap, MAX_MEMORY_LIMIT_IN_BYTES, SNOWPIPE_STREAMING_MAX_MEMORY_LIMIT_IN_BYTES);
            parameterOverrides.putAll(clientOverridePropertiesMap);
        });
        parameterOverrides.forEach((key, value) -> LOGGER.info("Streaming Client Config is overridden for {}={}", key, value));
    }

    private void overrideStreamingClientPropertyIfSet(Map<String, String> clientOverridePropertiesMap, String streamingClientPropName, String connectorPropName) {
        if (clientOverridePropertiesMap.containsKey(streamingClientPropName) && parameterOverrides.containsKey(streamingClientPropName)) {
            LOGGER.info("Honoring {} value in {} for streaming client provider, since it is" + " explicitly provided. Using: {}", connectorPropName, streamingClientPropName,
    ...
    public String getLoggableClientProperties() {
        return this.clientProperties == null | this.clientProperties.isEmpty() ? "" :
            this.clientProperties.entrySet().stream().filter(propKvp -> LOGGABLE_STREAMING_CONFIG_PROPERTIES.stream().anyMatch(propKvp.getKey().toString()::equalsIgnoreCase))
    ...
    public boolean equals(Object other) {
        return other.getClass().equals(StreamingClientProperties.class) && ((StreamingClientProperties) other).clientProperties.equals(this.clientProperties) &&
            ((StreamingClientProperties) other).parameterOverrides.equals(this.parameterOverrides) && ((StreamingClientProperties) other).isIcebergEnabled == this.isIcebergEnabled;
    }
