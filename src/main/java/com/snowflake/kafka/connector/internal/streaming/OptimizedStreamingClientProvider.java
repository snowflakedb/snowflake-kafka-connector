 ...
 */
public class OptimizedStreamingClientProvider implements StreamingClientProvider {
  private static volatile OptimizedStreamingClientProvider streamingClientProvider = null;
   ...
   */
  public static OptimizedStreamingClientProvider getStreamingClientProviderInstance() {
    if (streamingClientProvider == null) {
      synchronized (OptimizedStreamingClientProvider.class) {
        if (streamingClientProvider == null) {
          streamingClientProvider =
              new OptimizedStreamingClientProvider(clientHandlerSupplier.get());
        }
      }
    }
  ...
  private static final KCLogger LOGGER =
      new KCLogger(OptimizedStreamingClientProvider.class.getName());
   ...
   */
  private OptimizedStreamingClientProvider(StreamingClientHandler streamingClientHandler) {
  ...
  public SnowflakeStreamingIngestClient getClient(Map<String, String> connectorConfig) {
    ...
    LOGGER.info(
        "Streaming client optimization is enabled per worker node, KC will reuse valid clients when"
            + " possible. Returning client with name: {}",
  ...
  @VisibleForTesting
  public static OptimizedStreamingClientProvider getStreamingClientProviderForTests(
    ...
    return new OptimizedStreamingClientProvider(streamingClientHandler, registeredClients);
  }

  // TEST ONLY - private constructor to inject properties for testing
  @VisibleForTesting
  private OptimizedStreamingClientProvider(
      StreamingClientHandler streamingClientHandler,
      LoadingCache<StreamingClientProperties, SnowflakeStreamingIngestClient> registeredClients) {
    this.streamingClientHandler = streamingClientHandler;
