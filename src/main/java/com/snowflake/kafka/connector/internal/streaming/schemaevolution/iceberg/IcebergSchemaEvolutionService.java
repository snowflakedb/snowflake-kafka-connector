  ...
  public void evolveIcebergSchemaIfNeeded(
    ...
    if (!columnsToEvolve.isEmpty()) {
      ...
      IcebergTableSchema alreadyExistingColumns =
          tableSchemaResolver.resolveIcebergSchemaFromChannel(schemaAlreadyInUse, columnsToEvolve);

      IcebergTableSchema icebergSchemaFromRecord =
          tableSchemaResolver.resolveIcebergSchema(record, columnsToEvolve);
      ...
      TableSchema tableSchema =
          tableSchemaResolver.resolveTableSchemaFromRecord(record, columnsToEvolve);
      try {
        conn.appendColumnsToIcebergTable(tableName, tableSchema.getColumnInfos());
      } catch (SnowflakeKafkaConnectorException e) {
        LOGGER.warn(
