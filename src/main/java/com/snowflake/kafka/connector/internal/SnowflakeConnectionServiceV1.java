...
public class SnowflakeConnectionServiceV1 implements SnowflakeConnectionService {
    ...
    SnowflakeConnectionServiceV1(JdbcProperties jdbcProperties, SnowflakeURL url, String connectorName, String taskID, String kafkaProvider, IngestionMethodConfig ingestionMethodConfig) {
        ...
        this.internalStage = new SnowflakeInternalStage((SnowflakeConnectionV1) this.conn, credentialExpireTimeMillis, proxyProperties);
        this.telemetry = SnowflakeTelemetryServiceFactory.builder(conn, ingestionMethodConfig).setAppName(this.connectorName).setTaskID(this.taskID).build();
    ...
    public void createTable(final String tableName, final boolean overwrite) {
        ...
        String query;
        if (overwrite) {
            query = "create or replace table identifier(?) (record_metadata " + "variant, record_content variant)";
        } else {
            query = "create table if not exists identifier(?) (record_metadata " + "variant, record_content variant)";
        }
        try {
    ...
    public void createTableWithOnlyMetadataColumn(final String tableName) {
        checkConnection();
        InternalUtils.assertNotEmpty("tableName", tableName);
        String createTableQuery = "create table if not exists identifier(?) (record_metadata variant comment 'created by" + " automatic table creation from Snowflake Kafka Connector')";
        ...
        String enableSchemaEvolutionQuery = "alter table identifier(?) set ENABLE_SCHEMA_EVOLUTION = true";
        try {
            ...
            LOGGER.warn("Enable schema evolution failed on table: {}, message: {}", tableName, e.getMessage());
        }
    ...
    public void addMetadataColumnForIcebergIfNotExists(String tableName) {
        checkConnection();
        InternalUtils.assertNotEmpty("tableName", tableName);
        String query = "ALTER ICEBERG TABLE identifier(?) ADD COLUMN IF NOT EXISTS RECORD_METADATA " + ICEBERG_METADATA_OBJECT_SCHEMA;
        try {
            PreparedStatement stmt = conn.prepareStatement(query);
            stmt.setString(1, tableName);
            stmt.execute();
            ...
            LOGGER.error("Couldn't alter table {} add RECORD_METADATA column to align with iceberg format", tableName);
            throw SnowflakeErrors.ERROR_2019.getException(e);
        }
        LOGGER.info("alter table {} add RECORD_METADATA column to align with iceberg format", tableName);
    }
    ...
    public void initializeMetadataColumnTypeForIceberg(String tableName) {
        checkConnection();
        InternalUtils.assertNotEmpty("tableName", tableName);
        String query = "ALTER ICEBERG TABLE identifier(?) ALTER COLUMN RECORD_METADATA SET DATA TYPE " + ICEBERG_METADATA_OBJECT_SCHEMA;
        try {
            PreparedStatement stmt = conn.prepareStatement(query);
            stmt.setString(1, tableName);
            stmt.execute();
    ...
    @Override
    public void createPipe(final String tableName, final String stageName, final String pipeName, final boolean overwrite) {
        checkConnection();
        InternalUtils.assertNotEmpty("tableName", tableName);
    ...
    public boolean hasSchemaEvolutionPermission(String tableName, String role) {
        ...
        List<String> schemaEvolutionAllowedPrivilegeList = Arrays.asList("EVOLVE SCHEMA", "ALL", "OWNERSHIP");
        ResultSet result = null;
        ...
        try {
            ...
            while (result.next()) {
                ...
                if (schemaEvolutionAllowedPrivilegeList.contains(result.getString("privilege").toUpperCase())) {
        ...
        LOGGER.info(String.format("Table: %s has schema evolution permission: %s", tableName, hasPermission));
     ...
     */
    @Override
    public void alterColumnsDataTypeIcebergTable(String tableName, Map<String, ColumnInfos> columnInfosMap) {
    ...
    private String generateAlterSetDataTypeQuery(Map<String, ColumnInfos> columnsToModify) {
        ...
        String columnsPart = columnsToModify.entrySet().stream().map(column -> {
     ...
     */
    @Override
    public void appendColumnsToIcebergTable(String tableName, Map<String, ColumnInfos> columnInfosMap) {
        LOGGER.debug("Appending columns to iceberg table");
        appendColumnsToTable(tableName, columnInfosMap, true);
    }

    private void appendColumnsToTable(String tableName, Map<String, ColumnInfos> columnInfosMap, boolean isIcebergTable) {
        checkConnection();
        InternalUtils.assertNotEmpty("tableName", tableName);
        ...
        for (String columnName : columnInfosMap.keySet()) {
            ...
            appendColumnQuery.append(columnName).append(" ").append(columnInfos.getColumnType()).append(columnInfos.getDdlComments());
    ...
    public void alterNonNullableColumns(String tableName, List<String> columnNames) {
        ...
        for (String columnName : columnNames) {
            ...
            dropNotNullQuery.append(columnName).append(" drop not null, ").append(columnName)
                .append(" comment 'column altered to be nullable by schema evolution from Snowflake Kafka" + " Connector'");
        ...
        logColumn.insert(0, "Following columns' non-nullabilty was dropped for table {}:\n").append("]");
        LOGGER.info(logColumn.toString(), tableName);
    }
    ...
    private String pipeDefinition(String tableName, String stageName) {
        return "copy into " + tableName + "(RECORD_METADATA, RECORD_CONTENT) from (select $1:meta, $1:content from" + " @" + stageName + " t) file_format = (type = 'json')";
    }
    ...
    @VisibleForTesting
    protected ChannelMigrateOffsetTokenResponseDTO getChannelMigrateOffsetTokenResponseDTO(String migrateOffsetTokenResultFromSysFunc) throws JsonProcessingException {
        ChannelMigrateOffsetTokenResponseDTO channelMigrateOffsetTokenResponseDTO = OBJECT_MAPPER.readValue(migrateOffsetTokenResultFromSysFunc, ChannelMigrateOffsetTokenResponseDTO.class);
    ...
    public static class FormattingUtils {
        ...
        public static String formatName(String objectName) {
            return (objectName.charAt(0) == '"' && objectName.charAt(objectName.length() - 1) == '"') ? objectName.substring(1, objectName.length() - 1) : objectName.toUpperCase();
        }
    }
}
