...
public class SnowflakeConnectionServiceV1 implements SnowflakeConnectionService {
  ...
  public void addMetadataColumnForIcebergIfNotExists(String tableName) {
    ...
    InternalUtils.assertNotEmpty("tableName", tableName);
    String query =
        "ALTER ICEBERG TABLE identifier(?) ADD COLUMN IF NOT EXISTS RECORD_METADATA VARIANT";
    try {
      PreparedStatement stmt = conn.prepareStatement(query);
      stmt.setString(1, tableName);
      stmt.execute();
  ...
  public void initializeMetadataColumnTypeForIceberg(String tableName) {
    ...
    InternalUtils.assertNotEmpty("tableName", tableName);
    String query =
        "ALTER ICEBERG TABLE identifier(?) ALTER COLUMN RECORD_METADATA SET DATA TYPE VARIANT";
    try {
      PreparedStatement stmt = conn.prepareStatement(query);
      stmt.setString(1, tableName);
      stmt.execute();
