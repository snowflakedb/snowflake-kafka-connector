...
public interface TopicPartitionChannel {
   ...
   * invalidated while there are appended-but-uncommitted records and no new records arrive,
   * {@code appendRow} is never called again, so recovery never starts: {@code preCommit} keeps
   * skipping the failed offset fetch and the task is stuck (it neither ingests the last records
   * nor fails). The {@code preCommit} offset-fetch path calls this when it detects the client is
   * invalid so recovery starts even without new data.
   *
   * <p>No-op by default.
   */
