...
public class PipeNameProvider {
  ...
  public static String pipeName(Map<String, String> connectorConfig, String table) {
    ...
    // when the user has this option configured the user is expected to create the pipe and table
    // before starting connector. When this property is set pipe name and table name are expected
    // to have the same name. This allows to use interactive tables as destination tables because
    // interactive tables create the pipe with the same name as the table when user executes
