...
class StageFilesProcessorTest {
    ...
    void setUp() {
        ...
        register = new StageFilesProcessor.ProgressRegisterImpl(pipeCreation, pipeTelemetry, telemetryService);

        victim = new StageFilesProcessor(PIPE_NAME, TABLE_NAME, STAGE_NAME, PREFIX, conn, ingestionService, pipeTelemetry, telemetryService, currentTime::get);
    }
    ...
    void filesProcessWillTerminateOnStopSignal() throws InterruptedException {
        ...
        Thread worker = new Thread(() -> {
            victim.trackFiles(register, Thread.currentThread(), () -> !Thread.currentThread().isInterrupted(), d -> Thread.sleep(10L));
            completed.set(true);
        });
        worker.start();
    ...
    void fileProcessorWillTryToGetInitialStateFromStageExactlyOnce() {
        ...
        victim.trackFiles(register, Thread.currentThread(), () -> canRun.getAndDecrement() > 0, d -> {});
    ...
    void fileProcessorWillTryGettingStateFromStageOnError() {
        ...
        when(conn.listStage(STAGE_NAME, PREFIX)).thenReturn(new ArrayList<>());
        when(ingestionService.readIngestReport(reportRequest))
        ...
        verify(ingestionService, times(3)).readIngestReport(reportRequest);
    }
    ...
    void fileProcessorWillPurgeLoadedFiles() {
        ...
        when(conn.listStage(STAGE_NAME, PREFIX)).thenReturn(new ArrayList<>());
        when(ingestionService.readIngestReport(anyList()))
                .thenReturn(Collections.singletonMap(file1, InternalUtils.IngestedFileStatus.LOADED))
                .thenReturn(Collections.singletonMap(file2, InternalUtils.IngestedFileStatus.LOADED))
                .thenReturn(Collections.singletonMap(file3, InternalUtils.IngestedFileStatus.LOADED))
        ...
        doAnswer(a -> {
            ...
            return null;
        }).when(conn).purgeStage(anyString(), anyList());

        victim.trackFiles(register, Thread.currentThread(), () -> canRun.getAndDecrement() > 0, d -> {});

        verify(conn, times(1)).listStage(STAGE_NAME, PREFIX);
        verify(ingestionService, times(3)).readIngestReport(anyList());
    ...
    void fileProcessorWillMoveOldFilesToTableStage() {
        ...
        when(ingestionService.readIngestReport(anyList())).thenReturn(new HashMap<>());
        ...
        victim.trackFiles(register, Thread.currentThread(), () -> canRun.getAndDecrement() > 0, d -> {
            // advance time by 20 minutes - the last check will be 61 minutes after submitting files, so they should be
        ...
        // when files get to the 1 hour age, they will be automatically marked as failed and moved to table stage
