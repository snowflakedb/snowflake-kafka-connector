...
@ExtendWith({
  InjectSnowflakeDataSourceExtension.class,
  InjectQueryRunnerExtension.class,
  InjectAssertDbConnectionExtension.class
})
class UserDefinedPipeAndTableIT {
  ...
  private final Map<String, ?> testPayload =
      Map.of("city", "Pcim Górny", "age", 30, "married", false);
  ...
  @InjectAssertDbConnection private AssertDbConnection assertDb;

  @BeforeEach
  void setup() throws SQLException {
    config = TestUtils.getConfForStreaming();
    // Enable user defined table and pipe    config.put(SNOWPIPE_STREAMING_USE_USER_DEFINED_DATABASE_OBJECTS, "true");
    ...
    queryRunner.execute(
        format("create table %s (city varchar, age number, married boolean)", table));
    queryRunner.execute(
        format(
