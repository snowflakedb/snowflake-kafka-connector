...
public class ClientTaskMapTest {
    ...
    private void testConstructorAndGetTaskIdListsRunner(int taskCount, int numTasksPerClient, Set<List<Integer>> goalTaskIdLists) {
        ...
        ClientTaskMap clientTaskMap = new ClientTaskMap(taskCount, numTasksPerClient);
        Set<List<Integer>> gotTaskIdLists = clientTaskMap.getTaskIdLists();
    ...
    public void testConstructorInvalidParams() {
        assert TestUtils.assertError(SnowflakeErrors.ERROR_3011,
                () -> new ClientTaskMap(0, 100));

        assert TestUtils.assertError(SnowflakeErrors.ERROR_3011,
                () -> new ClientTaskMap(0, 0));

        assert TestUtils.assertError(SnowflakeErrors.ERROR_3011,
                () -> new ClientTaskMap(100, 0));

        assert TestUtils.assertError(SnowflakeErrors.ERROR_3011,
                () -> new ClientTaskMap(-100, 100));
    ...
    public void testGetAndAddClients() {
        ...
        ClientTaskMap clientTaskMap = new ClientTaskMap(5, 2);
        Set<SnowflakeStreamingIngestClient> goalClients = new HashSet<>();
        this.assertGoalClients(clientTaskMap, goalClients);

        // test add clientA
        clientTaskMap.upsertClient(clientATaskList, clientA);
        ...
        // test add clientB
        clientTaskMap.upsertClient(clientBTaskList, clientB);
        ...
        // test add clientC
        clientTaskMap.upsertClient(clientCTaskList, clientC);
    ...
    public void testGetAndRemoveClients() {
        ...
        ClientTaskMap clientTaskMap = new ClientTaskMap(5, 2);
        clientTaskMap.upsertClient(clientATaskList, existingClientA);
        clientTaskMap.upsertClient(clientBTaskList, existingClientB);
        clientTaskMap.upsertClient(clientCTaskList, existingClientC);
        this.assertGoalClients(clientTaskMap, goalClients);

        // test remove clientA
        clientTaskMap.removeClientTaskEntry(paramClientA);
        ...
        clientTaskMap.removeClientTaskEntry(notInitClient);

        Mockito.verify(notInitClient, Mockito.times(1)).getName();
        this.assertGoalClients(clientTaskMap, goalClients);

        // test remove clientB
        clientTaskMap.removeClientTaskEntry(existingClientB);
        goalClients.remove(existingClientB);
        this.assertGoalClients(clientTaskMap, goalClients);

        // test remove clientC
        clientTaskMap.removeClientTaskEntry(existingClientC);
...
//        Mockito.verify(clientC, Mockito.times(3)).getName();
    ...
    private void assertGoalClients(ClientTaskMap clientTaskMap, Set<SnowflakeStreamingIngestClient> goalClients) {
        Set<SnowflakeStreamingIngestClient> gotClients = clientTaskMap.getValidClients();
