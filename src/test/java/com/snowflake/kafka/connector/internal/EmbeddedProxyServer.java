...
public class EmbeddedProxyServer {
     ...
     *
     * <p>The proxy is configured using the same squid.conf file as CI (.github/scripts/squid.conf)
    ...
    public final void start() {
        ...
        try {
            // Verify squid.conf exists
            File squidConfFile = new File(SQUID_CONF_PATH);
            if (!squidConfFile.exists()) {
                throw new RuntimeException(
                    "Squid configuration file not found at: " + squidConfFile.getAbsolutePath());
            }
            ...
            proxyContainer =
                new GenericContainer<>(DockerImageName.parse("sameersbn/squid:3.5.27-2"))
                    ...
                    .withCopyFileToContainer(
                        MountableFile.forHostPath(squidConfFile.getAbsolutePath()),
    ...
    /**
     * Cleans up resources including container.
     */
