...
public class StreamingClientProviderTest {
  ...
  public void testGetInvalidClient() {
    ...
    StreamingClientProvider injectedProvider =
        injectStreamingClientProviderForTests(
            clientMap, injectedClient, this.streamingClientHandler);

    // test: getting invalid client
    this.clientConfig1.put(Utils.NAME, validClientName);
    SnowflakeStreamingIngestClient recreatedClient = injectedProvider.getClient(this.clientConfig1);
  ...
  public void testGetExistingClient() {
    ...
    SnowflakeStreamingIngestClient client2 =
        this.streamingClientProvider.getClient(this.clientConfig2);
    SnowflakeStreamingIngestClient client3 =
        this.streamingClientProvider.getClient(this.clientConfig3);
    SnowflakeStreamingIngestClient client4 =
        this.streamingClientProvider.getClient(this.clientConfig1);
  ...
  public void testCloseAllClients() throws Exception {
    ...
    StreamingClientProvider injectedProvider =
        injectStreamingClientProviderForTests(
            clientMap, parameterEnabledClient, this.streamingClientHandler);
    ...
    if (this.enableClientOptimization) {
      Mockito.verify(this.streamingClientHandler, Mockito.times(1))
          .closeClient(parameterEnabledClient);
    } else {
      clientMap
          .values()
          .forEach(
