...
public class SnowflakeSinkTaskMetricsTest {
  ...
  public void testAllMetricsRegistered() {
    ...
    assertNotNull(
        metricRegistry.getTimers().get(constructMetricName(PREFIX, TASK_SUB_DOMAIN, PUT_DURATION)));
    assertNotNull(
        metricRegistry
            .getTimers()
            .get(constructMetricName(PREFIX, TASK_SUB_DOMAIN, PRECOMMIT_DURATION)));
    assertNotNull(
        metricRegistry
            .getTimers()
            .get(constructMetricName(PREFIX, TASK_SUB_DOMAIN, PRECOMMIT_OFFSET_FETCH_DURATION)));

    // Lifecycle duration timers
    assertNotNull(
        metricRegistry
            .getTimers()
            .get(constructMetricName(PREFIX, LIFECYCLE_SUB_DOMAIN, OPEN_DURATION)));
    assertNotNull(
        metricRegistry
            .getTimers()
            .get(constructMetricName(PREFIX, LIFECYCLE_SUB_DOMAIN, CLOSE_DURATION)));
    assertNotNull(
        metricRegistry
            .getTimers()
            .get(constructMetricName(PREFIX, LIFECYCLE_SUB_DOMAIN, START_DURATION)));

    // Channel/SDK timers
    assertNotNull(
        metricRegistry
            .getTimers()
            .get(constructMetricName(PREFIX, LIFECYCLE_SUB_DOMAIN, CHANNEL_OPEN_DURATION)));
    assertNotNull(
        metricRegistry
            .getTimers()
            .get(constructMetricName(PREFIX, LIFECYCLE_SUB_DOMAIN, SDK_CLIENT_CREATE_DURATION)));

    // Meter
    assertNotNull(
        metricRegistry.getMeters().get(constructMetricName(PREFIX, TASK_SUB_DOMAIN, PUT_RECORDS)));

    // Counters
    assertNotNull(
        metricRegistry
            .getCounters()
            .get(constructMetricName(PREFIX, TASK_SUB_DOMAIN, PRECOMMIT_PARTITIONS_SKIPPED)));
    assertNotNull(
        metricRegistry
            .getCounters()
            .get(constructMetricName(PREFIX, LIFECYCLE_SUB_DOMAIN, OPEN_COUNT)));
    assertNotNull(
        metricRegistry
            .getCounters()
            .get(constructMetricName(PREFIX, LIFECYCLE_SUB_DOMAIN, CLOSE_COUNT)));
    assertNotNull(
        metricRegistry
            .getCounters()
            .get(constructMetricName(PREFIX, LIFECYCLE_SUB_DOMAIN, CHANNEL_OPEN_COUNT)));

    // Gauges
    assertNotNull(
        metricRegistry
            .getGauges()
            .get(constructMetricName(PREFIX, TASK_SUB_DOMAIN, ASSIGNED_PARTITIONS)));
  }
  ...
  public void testSdkClientCountGauge() {
    ...
    Gauge<Integer> gauge =
        metricRegistry
            .getGauges()
            .get(constructMetricName(PREFIX, LIFECYCLE_SUB_DOMAIN, SDK_CLIENT_COUNT));
    assertNotNull(gauge);
