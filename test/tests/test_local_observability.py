...
def test_prometheus_metrics_exposed(
    ...
    4. Scrape http://{PROM_HOST}:{PROM_PORT}/metrics and assert every metric in
       EXPECTED_SDK_METRICS appears in the response body. The Connect-worker container
       enables the SDK metrics endpoint (SS_ENABLE_METRICS) so it is reliably up in the
       shared-worker test suite.
    """
