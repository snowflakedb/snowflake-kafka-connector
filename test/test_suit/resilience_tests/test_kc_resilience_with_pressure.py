...
class TestKcResilienceWithPressure:
    ...
    def send(self):
        print("Resilience tests with pressure (send data) for Kafka Connector. Note: this test uses verify() as a runner to test how different methods interact, so most of the logic is within the verify block.\n"
              "Each iteration upserts a new connector, sends data, tests out the methods with pressure, sends data again and finally verifies that the expected data arrived in the table\n"
              "iteration 1: pause, send data, then resume connector\n"
              "iteration 2: delete, send data, then create connector\n"
              "iteration 3: delete, send data, then resume connector - second and third rounds of data should be missing\n"
              "iteration 4: pause, send data, then create connector\n")

    def verify(self, round):
        # continue test iterations
        if self.testIt < self.totalIt:
            ...
            if self.testIt % self.totalIt == 1:
                ...
                sleep(self.sleepTime)
                self.driver.resumeConnector(self.connectorName)
                expectedsends = expectedsends - 1 # resume shouldn't create the connector, so miss the last sendbytes
