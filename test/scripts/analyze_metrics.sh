...
def parse_mbean(key):
    """Extract connector, pipe, category, name from an MBean key."""
    parts = {}
...
for key, val in metrics.items():
    ...
    if pipe.startswith("task-"):
        if name in TIMER_NAMES:
            task_timers[pipe][name] = val
        elif name in METER_NAMES:
            task_meters[pipe][name] = val
        elif name in COUNTER_NAMES:
            task_counters[pipe][name] = val
        elif name in GAUGE_NAMES:
            task_gauges[pipe][name] = val
    elif cat == "offsets":
        if name in OFFSET_NAMES:
            channel_gauges[pipe][name] = val
        elif name in CHANNEL_COUNTER_NAMES:
            channel_counters[pipe][name] = val
