...
public class Utils {
  ...
  public static boolean TEST_MODE =
      Boolean.parseBoolean(System.getenv().getOrDefault(LOCAL_TEST, "true"));
  ...
  public static void createTestTopic() {
    runCommand(
        "./confluent/bin/kafka-topics --create --zookeeper "
  ...
  public static void startConnector(Formats format) {
    ...
    config.put("snowflake.user.name", server.get(ConnectionUtils.USER).asText());
    config.put("snowflake.private.key", server.get(ConnectionUtils.PRIVATE_KEY).asText());
    config.put("snowflake.database.name", server.get(ConnectionUtils.DATABASE).asText());
    config.put("snowflake.schema.name", server.get(ConnectionUtils.SCHEMA).asText());
