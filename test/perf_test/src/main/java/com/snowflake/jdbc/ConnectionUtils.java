...
public class ConnectionUtils {
  ...
  public static void uploadReport() {
    runQuery("create temp stage " + RESULT_STAGE);

    runQuery("create table if not exists " + RESULT_TABLE + " ( KAFKA_PERF_TEST_RESULT VARIANT )");

    runQuery("put file://" + Utils.REPORT_FILE_PATH + " @" + RESULT_STAGE);
  ...
  private static void initConnection() {
    ...
    try {
      testConn = new SnowflakeConnectionV1(testProps.getProperty(URL), testProps);
      //      snowhouseConn =
  ...
  public static int tableSize() {
    ResultSet result = runQuery("show tables like '" + Utils.TEST_TOPIC + "'");
    try {
      if (result.next()) {
